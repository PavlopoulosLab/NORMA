...
def test_string_ping(norma):
    ...
    assert status == 200 and obj["norma_proxy"] is True and obj["upstream"] == server.DEFAULT_UPSTREAM


def test_string_relay_get_becomes_upstream_post(norma, upstream):
    status, h, obj = norma.json("/string-api/json/network?identifiers=TP53&species=9606&upstream=" + upstream)
    ...
    assert obj["user_agent"] == server.USER_AGENT
...
def test_string_relay_post_merges_query_into_form(norma, upstream):
    status, _, obj = norma.json(
        ...
        method="POST", body=b"identifiers=A%0dB", headers={"Content-Type": "application/x-www-form-urlencoded"},
    )
...
def test_string_allowed_upstream_regex():
    ...
    bad = ("https://string-db.org.evil.com", "https://notstring-db.org", "http://string-db.org", "https://string-db.orgx")
    assert all(server.ALLOWED_UPSTREAM.match(u) for u in ok)
    assert not any(server.ALLOWED_UPSTREAM.match(u) for u in bad)


def test_string_relay_body_limit(norma, upstream, monkeypatch):
    monkeypatch.setattr(server, "MAX_BODY", 10)
    status, _, _ = norma.json("/string-api/json/network?upstream=" + upstream, method="POST", body=b"identifiers=" + b"x" * 50)
    assert status == 413



def test_relay_unreachable_upstream_502(norma, monkeypatch):
    monkeypatch.setattr(server, "TEST_UPSTREAM", "http://127.0.0.1:1")
...
def test_string_relay_off(norma):
    server.CONFIG["relays"]["string"] = False
...
def test_arena_relay(norma, upstream):
    ...
    status, _, obj = norma.json("/arena3d-api/external?upstream=" + upstream, method="POST", body=payload)
...
def test_arena_relay_validation(norma, upstream, monkeypatch):
    assert norma.json("/arena3d-api/external?upstream=https://evil.org", method="POST", body={})[0] == 400
    status, _, _ = norma.request("/arena3d-api/external?upstream=" + upstream, method="POST", body=b"nope", headers={"Content-Type": "application/json"})
    assert status == 400
    monkeypatch.setattr(server, "MAX_ARENA_BODY", 5)
    assert norma.json("/arena3d-api/external?upstream=" + upstream, method="POST", body={"a": "long"})[0] == 413
    server.CONFIG["relays"]["arena3d"] = False
    assert norma.json("/arena3d-api/external", method="POST", body={})[0] == 404


def test_arena_allowed_regex():
    assert server.ALLOWED_ARENA.match("https://arena3d.org")
    assert server.ALLOWED_ARENA.match("https://beta.arena3d.org")
    assert not server.ALLOWED_ARENA.match("https://arena3d.org.evil.com")


def test_db_relay_get_and_post(norma, upstream):
    target = upstream + "/db/pathway?id=1"
    status, h, obj = norma.json("/db-api/fetch?url=" + target.replace("&", "%26").replace("?", "%3F"))
    assert status == 200 and h["X-Norma-Relay"] == "1"
    assert obj["method"] == "GET" and obj["path"] == "/db/pathway?id=1"
    status, _, obj = norma.json("/db-api/fetch?url=" + upstream + "/db/q", method="POST", body={"q": 1})
...
def test_db_relay_allowlist(norma, monkeypatch):
    ...
    for good in (
        ...
        assert any(p.match(good) for p in server.ALLOWED_DB), good
    assert not any(p.match("https://reactome.org.evil/ContentService/") for p in server.ALLOWED_DB)
    monkeypatch.setattr(server, "MAX_DB_BODY", 3)
    assert norma.json("/db-api/fetch?url=https://omnipathdb.org/x", method="POST", body={"big": "x"})[0] == 413
    server.CONFIG["relays"]["databases"] = False
