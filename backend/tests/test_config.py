...
def test_hosted_defaults(monkeypatch, tmp_path):
    monkeypatch.setattr(config, "ROOT", str(tmp_path))
    cfg = config.load_config(make_args(mode="hosted"))
...
def test_precedence_file_env_cli(monkeypatch, tmp_path):
    ...
    f.write_text(
        json.dumps(
            {
                "mode": "hosted",
                "server": {"port": 9000, "publicUrl": "https://x.org"},
                "site": {"name": "X"},
            }
        )
    )
    ...
    cfg = config.load_config(make_args(config=str(f), port=9200))
    assert cfg["server"]["port"] == 9200  # cli > env > file
    assert cfg["server"]["publicUrl"] == "https://x.org/"  # trailing slash added
    assert (
        cfg["site"]["name"] == "X" and cfg["site"]["contactName"] == "Pavlopoulos Lab"
    )  # deep merge
...
def test_cli_switches(monkeypatch, tmp_path):
    ...
    cfg = config.load_config(
        make_args(no_api=True, no_relays=True, no_open=True, access_log="full")
    )
    assert cfg["api"]["enabled"] is False
...
def test_missing_config_file_exits():
    with pytest.raises(SystemExit):
        config.load_config(make_args(config="/nonexistent.json"))
...
def test_client_config_has_no_server_section():
    cfg = config.load_config(make_args(mode="hosted", public_url="https://n.org"))
    cc = config.client_config(cfg)
...
def test_repo_config_files_load():
    ...
    for name in ("norma.config.json", "norma.config.hosted.json"):
        cfg = config.load_config(make_args(config=os.path.join(root, name)))
