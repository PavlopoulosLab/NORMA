...
- [x] 1. `test(backend): pytest suite for server.py` — fixtures: temp APP_DIR, fake upstream servers, `NORMA_*_TEST_UPSTREAM` env
- [x] 2. `test(e2e): Playwright smoke for norma.html` — `frontend/e2e/`, `window.__norma` hook added to norma.html
- [x] 3. `docs: LICENSE (MIT), AGENTS.md, CLAUDE.md, CHANGELOG.md`
- [x] 4. `refactor(backend): split server.py into backend/norma package` + pyproject, uv, ruff, mypy; tests moved
